	...
	try:
		...
		if len(qc_filename) > 0:
			...
			if 'msing_score' in qc_df_data.columns:
				...
				msi_list = msi_arr[0] if len(msi_arr) > 0 else 'N/A'
			...
			qc_df_data.fillna('N/A', inplace=True)
	...
	try:
		...
		for i in smv_file_list:
			...
			if(re.match(regex, i)):
				...
				smv_df_data['ref_reads'] = smv_df_data['ref_reads'] - smv_df_data['alt_reads']
				smv_df_data["clonality"] = smv_df_data["clonality"].apply(lambda x: x.capitalize()) if not smv_df_data["clonality"].isna().any() else smv_df_data["clonality"]
		...
		smv_df.fillna('N/A', inplace=True)
...
def build_cnv(root_path):
	...
	try:
		...
		cnv_df.fillna('N/A', inplace=True)
...
def build_svs(root_path):
	...
	try:
		...
		column_dict = {'SAMPLE': 'origin', 'SVTYPE': 'sv_type', 'GENE_A': 'gene_a' , 'IGV_COORD' : 'variant', 'GENE_B' : 'gene_b'}
		...
		if not svs_filter.empty:
			...
			if 'CONSEQUENCE' in svs_filter.columns:
				...
				svs_filter["consequence"] = 'N/A'

			if 'SECONDHIT' in svs_filter.columns:
				column_list.append('SECONDHIT')
				column_dict['SECONDHIT'] = 'secondhit'
			else:
				svs_filter["secondhit"] = "N/A"

			if 'CLONALITY' in svs_filter.columns:
				column_list.append('CLONALITY')
				column_dict['CLONALITY'] = 'clonality'
			else:
				svs_filter["clonality"] = "N/A"
			...
			svs_filter.fillna('N/A', inplace=True)

			svs_filter["origin"] = svs_filter["origin"].apply(lambda x: x.capitalize())	
			svs_filter["clonality"] = svs_filter["clonality"].apply(lambda x: x.capitalize()) if svs_filter["clonality"].isna().any() else svs_filter["clonality"]
			svs_filter["consequence"] = svs_filter["consequence"].apply(lambda x: x.capitalize()) if svs_filter["consequence"].isna().any() else svs_filter["consequence"]
...
def build_json(root_path, output_path, project_name, cfdna, sample_id, capture_format):
	...
	file_name = output_path+"/MTBP_"+capture_format+"_"+identifier_study_id+"_CFDNA"+cfdna+"_all.json"

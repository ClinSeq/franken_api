...
This module defines to generate the pdf for the Franken API project.
"""
...
def build_specimen(specimen_assay):
    ...
    for i in specimen_assay_json:
        ...
        if "quality" in specimen_assay_json[i]:
            quality = (
                specimen_assay_json[i]["quality"]
                if isinstance(specimen_assay_json[i]["quality"], str)
                else ""
            )
            if quality == "Pass":
        ...
        specimen_assay = (
            specimen_assay_json[i]["assay"] if "assay" in specimen_assay_json[i] else ""
        )
...
def build_genomic_feature(genomic_wide):
    ...
    for j in genome_wide_json:
        ...
        if "title" in genome_wide_json[j]:
            genome_title = (
                genome_wide_json[j]["title"]
                if "title" in genome_wide_json[j]
                else genome_wide_json[j]
            )
            genome_result = (
                genome_wide_json[j]["result"] if "result" in genome_wide_json[j] else ""
            )
            ...
            if "assessment" in genome_wide_json[j]:
                assessment = (
                    "Yes"
                    if genome_wide_json[j]["assessment"] == "Possible"
                    else (
                        "No"
                        if genome_wide_json[j]["assessment"] == "Not possible"
                        else ""
                    )
                )
            ...
            if ("assessment" in genome_wide_json[j]) and (
                genome_wide_json[j]["title"] == "OTHER GENOMIC PHENOTYPE"
                and genome_wide_json[j]["assessment"] == "Possible"
                and result_data == "Yes"
            ):
    ...
    genome_wide_txt += (
        "KONSTITUTIONELLA (POTENTIELLT ÄRFTLIGA) VARIANTER: \t{}\n\n".format(
            potentiellt_ärftliga
        )
    )
    ...
    try:
        ...
        for i in smv_file_list:
            ...
            if "CALL" in smv_df_data.columns:
                smv_df_call_filter = smv_df_data.loc[
                    (smv_df_data["CALL"] == "S") | (smv_df_data["CALL"] == "G")
                ]

                if "include_variant_report_pdf" in smv_df_call_filter.columns:
                    smv_df_call_filter = smv_df_call_filter.loc[
                        (smv_df_call_filter["include_variant_report_pdf"] == "True")
                        | (smv_df_call_filter["include_variant_report_pdf"] == True)
                    ]
                ...
                smv_df_filter_data = smv_df_filter_data.sort_values(
                    sort_arr, ascending=True
                )
                ...
                if len(smv_df_filter_data):
                    ...
                    for index, row in smv_df_filter_data.iterrows():
                        ...
                        HGVSp_org = (
                            HGVSp_org_rx.split("p.")[1]
                            if "p." in HGVSp_org_rx
                            else HGVSp_org_rx
                        )
                        second_hit = (
                            str(row["second_hit"]) if "second_hit" in row else "-"
                        )
                        ...
                        smt_variant_html += (
                            '<td class="sm-var-dets"><p>' + variant_det + "</p></td>"
                        )
            ...
            for i in cnv_file_list:
                ...
                if "ASSESSMENT" in cnv_df_data.columns:
                    ...
                    if "include_variant_report_pdf" in cnv_df_data.columns:
                        cnv_df_data = cnv_df_data.loc[
                            (cnv_df_data["include_variant_report_pdf"] == "True")
                            | (cnv_df_data["include_variant_report_pdf"] == True)
                        ]
                    ...
                    for index, row in cnv_df_data.iterrows():
                        ...
                        copy_number = (
                            row["copy_number"]
                            if isinstance(row["copy_number"], str)
                            else int(row["copy_number"])
                        )
    ...
    try:
        ...
        if "CALL" in svs_filter.columns:
            ...
            if "include_variant_report_pdf" in svs_filter.columns:
                svs_filter = svs_filter.loc[
                    (svs_filter["include_variant_report_pdf"] == "True")
                    | (svs_filter["include_variant_report_pdf"] == True)
                ]
            ...
            if not svs_filter.empty:
                ...
                for index, row in svs_filter.iterrows():
                    ...
                    start_b = (
                        str(int(row["START_B"])) + "-" if row["START_B"] != "-" else ""
                    )
...
def build_tech_val_QC(root_path, project_name, capture_id):
    ...
    try:
        ...
        specimen = (
            "cfDNA"
            if "CFDNA" in capture_arr[0]
            else ("FFPE" if "T" in capture_arr[0] else "Tumor")
        )
        ...
        if len(qc_filename) > 0:
            ...
            tech_html += (
                "<td>" + str(math.ceil(int(qc_df_json["coverage"][0]))) + "</td>"
            )
            if df_len > 1:
                tech_html += (
                    "<td>" + str(math.ceil(int(qc_df_json["coverage"][1]))) + "</td>"
                )
            else:
                tech_html += "<td>-</td>"
            ...
            if df_len > 1:
                tech_html += (
                    "<td>" + str(round(qc_df_json["duplication"][1], 2)) + "</td>"
                )
            else:
                tech_html += "<td>-</td>"
            tech_html += "</tr>"
...
def build_clinical_txt(project_path, study_code, report_txt):
    ...
    file_name = os.path.join(
        project_path, "autoseq_clinical_report_" + study_code + ".txt"
    )
...
def build_html(
    ...
    report_txt = ""
    try:
        sql = "SELECT study_code, study_site, dob, disease, specimen_assay, genome_wide, summary_txt from genomic_profile_summary WHERE sample_id='{}' and capture_id='{}' and project_name='{}' order by id desc limit 1".format(
            sample_id, capture_id, project_name
        )
...
def generate_pdf(project_path, project_name, sample_id, capture_id):
    ...
    logging.info(
        "Sample Id : {} || Capture Id : {} || Outpue File Name : {} ".format(
            sample_id, capture_id, base_html_file
        )
    )

    try:

        html_status, html_text = build_html(
            root_path,
            output_pdf_path,
            base_html_path,
            html_root_path,

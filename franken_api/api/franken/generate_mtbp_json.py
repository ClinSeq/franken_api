...
def create_db_session(db_name, query):
    ...
    try:
        ...
        engine = (
            db.get_engine(current_app, db_name)
            if db_name != ""
            else db.get_engine(current_app)
        )
...
def fetch_cancer_type_code(disease):
    cancer_code = "NA"
    tissue = "NA"
    sql = "SELECT ts.sub_type_code as cancer_code, tt.tissue_name  FROM ipcm_tissue_subtype as ts INNER JOIN ipcm_tissue_type as tt ON tt.t_id=ts.t_id where ts.sub_type_name ~* '^({})$' order by ts.sub_type_level asc limit 1".format(
        disease
    )
...
def build_ipcm_sample_details(
    normal_cfdna, cfdna, capture_format, sample_type, seq_date, germline_dna
):
    ...
    try:
        ...
        sql2 = "select rf.pnr, rf.cdk from ipcm_referral_t as rf WHERE rf.rid like'%{}%' OR rf.blood like'%{}%' OR rf.dna1 like'%{}%' OR rf.dna2 like'%{}%' OR rf.dna3 like'%{}%'".format(
            normal_cfdna, normal_cfdna, normal_cfdna, normal_cfdna, normal_cfdna
        )
        ...
        res_data[0]["cdk"] if len(res_data) > 0 else ""
        ...
        if t_pnr == n_pnr or t_pnr == "":
            ...
            if len(res_ecrd_data) > 0:
                ...
                json_data = json.loads(
                    res_json, parse_int=lambda x: int(x) if x.isdigit() else x
                )
                identifier_status = True
                res_json_data = json_data[0]
                res_json_data["tissue_type"] = validate_tissue_type(
                    res_json_data["tissue_type"], sample_type
                )
                res_json_data["pathology_ccf"] = (
                    "NA"
                    if res_json_data["pathology_ccf"] == None
                    else res_json_data["pathology_ccf"]
                )
            else:
                identifier_status = False
        else:
            pnr = t_pnr
            query_ecrf_2 = "SELECT CONCAT('MTBP_{}_', ec.study_id,'_{}{}') as identifier, TO_DATE(rf.datum::text, 'YYYYMMDD') as referral_date, '{}' as seq_date, TO_DATE(rf.date_birth::text, 'YYYYMMDD') as birthdate, get_hospital_code(ec.site_id) as hospital, 'oncotree' as cancer_taxonomy, CASE WHEN ec.cancer_type_id != 0 THEN get_tissue_name(ec.cancer_type_id,ec.cancer_type_code) ELSE 'NA' END as tissue, CASE WHEN ec.cancer_type_code !='' THEN ec.cancer_type_code ELSE 'NA' END as cancer_code, 'primary' as tissue_source, CASE WHEN ec.tissue_type ='' THEN 'NA' ELSE ec.tissue_type END as tissue_type, CASE WHEN cell_fraction <> '' THEN CAST(cell_fraction AS FLOAT) ELSE NULL END AS pathology_ccf, CASE WHEN ec.germline_dna = '0' THEN {} ELSE to_number(ec.germline_dna::text, '9'::text)::integer END as germline_dna from ipcm_referral_t as rf INNER JOIN ipcm_ecrf_t as ec ON regexp_replace(CAST(ec.birth_date AS VARCHAR), '-', '', 'g') =  LEFT(rf.pnr, 8)  WHERE rf.pnr='{}'".format(
                capture_format, sample_type, cfdna, seq_date, germline_dna, pnr
            )
...
def build_sample_details(
    project_name, capture_id, cfdna, capture_format, sample_type, seq_date, germline_dna
):
    ...
    sql = "SELECT pnr, CAST(datum AS VARCHAR) as datum, rid, tid from probio_bloodreferrals WHERE cf_dna1 like '%{}%' OR cf_dna2 like '%{}%' or cf_dna3 like '%{}%' or kommentar like '%{}%'".format(
    ...
    if res_data:
        ...
        sql2 = "SELECT subject_id, CAST(dob as VARCHAR), site_name from sample_status_t WHERE pnr like '%{}%'".format(
            pnr
        )
        ...
        sql_bio = "SELECT DISTINCT rid, subjectid from biobank_t WHERE regexp_replace(referenceID, '[^a-zA-Z0-9]+', '','g') like '{}' or regexp_replace(referenceID, '[^a-zA-Z0-9]+', '','g') like '{}'  or regexp_replace(referenceID, '[^a-zA-Z0-9]+', '','g') IN ('{}') or regexp_replace(rid, '[^a-zA-Z0-9]+', '','g') IN('{}')".format(
        ...
        if bio_data:
            ...
            sql_glb_2 = "SELECT subject_id, CAST(dob as VARCHAR), site_name from sample_status_t WHERE regexp_replace(subject_id, '[^a-zA-Z0-9]+', '','g') like regexp_replace('{}', 'P-', '','g') or regexp_replace(subject_id, '[^a-zA-Z0-9]+', '','g') like regexp_replace('{}', '-', '','g')".format(
...
def build_genomic_profile_sample_details(
    project_name,
    cfdna,
    sample_id,
    capture_id,
    ...
    sql = "SELECT study_code, study_site, dob, disease FROM genomic_profile_summary where project_name='{}' and sample_id='{}' and capture_id='{}'".format(
        project_name, sample_id, capture_id
    )
    ...
    if res_data:
        for key, val in enumerate(res_data):
            ...
            if dob and dob != "NA":
                ...
                sample_data["birthdate"] = (
                    datetime.strptime(dob.strip(), dob_format)
                    .date()
                    .strftime("%Y-%m-%d")
                )
            if study_site and study_site != "NA":
                ...
                sample_data["hospital"] = (
                    hospital_lookup[study_site]
                    if (study_site in hospital_lookup)
                    else study_site
                )
...
def fetch_cancer_code(project_name, sample_id, capture_id):
    cancer_code = ""
    sql = "SELECT study_code, disease FROM genomic_profile_summary where project_name='{}' and sample_id='{}' and capture_id='{}' limit 1".format(
        project_name, sample_id, capture_id
    )
    res = create_db_session("curation", sql)
    res_data = generate_list_to_dict(res)
...
def build_qc(root_path, ecrf_tissue_type, sample_type):
    ...
    try:
        qc_file_list = list(
            filter(
                lambda x: x.endswith(".qc_overview.txt")
                ...
                for idx, row in qc_df_data.iterrows():
                    ...
                    if "-N-" in sample_name:
                        ...
                        if ecrf_tissue_type != "NA" and ecrf_tissue_type != "":
                            qc_df_data.loc[idx, "tissue_type"] = validate_tissue_type(
                                ecrf_tissue_type, sample_type
                            )
                        else:
...
def build_ploidy(root_path):
    ...
    regex = r"[-\w]+-(CFDNA|T)-[A-Za-z0-9-]+.csv"
    ...
    try:
        if os.path.exists(file_path):
            purecn_file_list = list(
                filter(lambda x: (re.match(regex, x)), os.listdir(file_path))
            )
            if len(purecn_file_list) > 0:
                ...
                if os.stat(purecn_filename).st_size != 0:
                    ...
                    if purecn_df_data.empty:
                        purecn_df_data = purecn_df_data[
                            ["Sampleid", "Purity", "Ploidy", "Sex", "Contamination"]
                        ]
...
def build_penotypes(project_name, sample_id, capture_id):
    ...
    sql = "SELECT ctdna_param, ctdna_method, genome_wide FROM genomic_profile_summary where project_name='{}' and sample_id='{}' and capture_id='{}'".format(
        project_name, sample_id, capture_id
    )
    res = create_db_session("curation", sql)
    res_data = generate_list_to_dict(res)
    ...
    if len(res_data) > 0:
        ...
        for j in genome_wide_json:
            ...
            if title == "FRACTION OF CANCER DNA":
                purity_val = (
                    genome_wide_json[j]["result"]
                    if (
                        "result" in genome_wide_json[j]
                        and genome_wide_json[j]["result"] != []
                        and genome_wide_json[j]["result"] != ""
                    )
                    else "NA"
                )
                ...
                tumor_arr = (
                    genome_wide_json[j]["comment"]
                    if (
                        "comment" in genome_wide_json[j]
                        and genome_wide_json[j]["comment"] != []
                        and genome_wide_json[j]["comment"] != ""
                    )
                    else "NA"
                )
...
def build_small_variants(root_path):
    ...
    try:
        smv_file_list = list(
            filter(
                lambda x: x.endswith("-igvnav-input.txt")
                and not x.startswith(".")
                and not x.endswith(".out"),
                os.listdir(file_path),
            )
        ...
        for i in smv_file_list:
            ...
            if "CALL" in smv_df_data.columns:
                ...
                smv_df_data = smv_df_data.loc[
                    (smv_df_data["CALL"] == "S") | (smv_df_data["CALL"] == "G")
                ]
                ...
                if re.match(regex, i) or re.match(regex2, i):
                    ...
                    smv_df_data["ref_reads"] = (
                        smv_df_data["ref_reads"] - smv_df_data["alt_reads"]
                    )
                else:
                ...
                smv_df_data["clonality"] = (
                    smv_df_data["clonality"].apply(lambda x: x.capitalize())
                    if "clonality" in smv_df_data.columns
                    else ""
                )

                smv_df_data["alt"] = smv_df_data["alt"].map(
                ...
                if "second_hit" in smv_df_data.columns:
                    smv_df_data["second_hit"] = smv_df_data["second_hit"].replace(
                        "-", "NA"
                    )
...
def build_cnv(root_path):
    ...
    try:
        cnv_file_list = list(
            filter(
                lambda x: x.endswith("_curated.cns")
                and not x.startswith(".")
                and not x.endswith(".out"),
                os.listdir(file_path),
            )
            ...
            for i in cnv_file_list:
                ...
                if "ASSESSMENT" in cnv_df_data.columns:
                    ...
                    if cnv_df_data.empty:
                        ...
                        cnv_df_data["copy_number"] = cnv_df_data["copy_number"].fillna(
                            0
                        )
                        ...
                        cnv_df_data["genes"] = cnv_df_data["genes"].apply(
                            lambda x: "NA" if pd.isna(x) else x.split(", ")
                        )
...
def build_svs(root_path):
    ...
    try:
        svs_file_list = list(
            filter(
                lambda x: (
                    re.match(r"[-\w]+-(CFDNA|T)-[A-Za-z0-9-]+-sv-annotated.txt", x)
                    or x.endswith("_annotate_combined_SV.txt")
                )
                and not x.startswith(".")
                and not x.endswith(".out"),
                os.listdir(file_path),
        ...
        if len(svs_file_list) > 0:
            ...
            if "CALL" in svs_filter.columns:
                svs_filter = svs_filter.loc[
                    (svs_filter["CALL"] == True) | (svs_filter["CALL"] == "true")
                ]
                ...
                if not svs_filter.empty:
                    ...
                    svs_filter["SAMPLE"] = np.where(
                        svs_filter["SAMPLE"] == "germline", "germline", "somatic"
                    )
                    ...
                    svs_filter["origin"] = svs_filter["origin"].apply(
                        lambda x: x.capitalize()
                    )
                    svs_filter["clonality"] = svs_filter["clonality"].apply(
                        lambda x: x.capitalize()
                    )
                    svs_filter["consequence"] = svs_filter["consequence"].apply(
                        lambda x: x.capitalize()
                    )
...
def build_json(
    ...
    if project_name == "IPCM" or capture_format == "iPCM":
        sample_details_json, itendifiter_status = build_ipcm_sample_details(
            normal_cfdna, cfdna, capture_format, sample_type, seq_date, germline_dna
        )
    elif project_name != "PREDDLUNG":
        sample_details_json, itendifiter_status = build_sample_details(
            project_name,
            capture_id,
            cfdna,
            capture_format,
    ...
    if sample_details_json and itendifiter_status:
        ...
        sample_details_json = build_genomic_profile_sample_details(
            project_name,
            cfdna,
            sample_id,
            capture_id,
    ...
    if sample_details_json["identifier"] == "NA":
        if project_name == "PREDDLUNG":
            ...
            sample_details_json["identifier"] = "MTBP_PreDDLung_{}_{}{}".format(
                identifier_study_id, sample_type, cfdna
            )
        else:
    ...
    if sample_details_json["cancer_code"] == "NA":
        ...
        if disease != "":
            ...
            sample_details_json["cancer_code"] = (
                cancer_code_new
                if cancer_code_new != "NA"
                else sample_details_json["tissue"]
            )
    ...
    # Phenotype
    purity_val, ploidy_val, msi, tmb, td, pathogenic_gDNA_variant, tumor_val = (
        build_penotypes(project_name, sample_id, capture_id)
    )
    project_json["phenotypes"] = {
        "purity": convert_to_numeric(purity_val),
        "ploidy": convert_to_numeric(ploidy_val),
        "msi": msi,
        "tmb": tmb,
        "td": td,
        "pathogenic_gDNA_variant": pathogenic_gDNA_variant,
        **(tumor_val if isinstance(tumor_val, dict) else {}),
    }
...
def generate_json(nfs_path, project_name, sample_id, capture_id):
    ...
    cfdna_idx = (
        capture_arr.index("T") if "T" in capture_arr else capture_arr.index("CFDNA")
    )
